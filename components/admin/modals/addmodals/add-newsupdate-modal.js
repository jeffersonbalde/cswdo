class AddNewsupdateModal extends HTMLElement {
  constructor() {
    super();
  ...
  async connectedCallback() {
    ...
    await this.fetchNewNewsupdateId(); // 🆕 fetch from backend
  ...
  initializeElements() {
    ...
    this.modal = this.shadowRoot.getElementById("addNewsupdateModal");
    this.form = this.shadowRoot.getElementById("addNewsupdateForm");
    this.cancelBtn = this.shadowRoot.getElementById("cancelAddBtn");
    this.closeBtn = this.shadowRoot.getElementById("closeAddModal");
    //Inputs
    this.newsupdateId = this.shadowRoot.getElementById("newsupdateId"); // Added for News/Update ID
    ...
    this.errorDiv = this.shadowRoot.getElementById("addNewsupdateError");
  }
  ...
  hasUserInput() {
    // News/Update ID and upload date are auto-filled; ignore them for user input detection
    const titleHas = (this.newsupdateTitle?.value || "").trim().length > 0;
    const descHas = (this.newsupdateDesc?.value || "").trim().length > 0;
  ...
  async fetchNewNewsupdateId() {
    try {
      ...
      if (json.success) {
        this.newsupdateId.value = json.newsupdateId;
        this.newsupdateId.readOnly = true;
      } else {
        this.showError("Failed to load News/Update ID.");
      }
    } catch (err) {
      console.error("Error fetching ID:", err);
      this.showError("Could not get News/Update ID.");
    }
  }
  ...
  async handleSubmit() {
    ...
    if (this.selectedFile) {
      formData.append("newsupdateImage", this.selectedFile);
    }
    ...
    if (!this.selectedFile) {
      this.showError("Please select a news/update image.");
      return;
    }
      ...
      if (e.detail.confirmed) {
        ...
        try {
          ...
          if (result.success) {
            ...
            this.dispatchEvent(
              new CustomEvent("newsupdate-saved", {
                detail: { newsupdate: data },
                bubbles: true,
              })
            );
  ...
  render() {
    this.shadowRoot.innerHTML = `
              ...
              <div id="addNewsupdateModal" class="modal">
                  <div class="modal-dialog">
                      <div class="modal-content">
                          <div class="modal-bg-logo"></div>
                          <div class="modal-header">  
                              <h5 id="addNewsupdateTitle" class="modal-title">Add News and Updates</h5>
                              <button type="button" id="closeAddModal" class="btn-close">&times;</button>
                          </div>
                          <div class="modal-body">
                              <div id="addNewsupdateError" class="alert alert-danger"></div>
                              <form id="addNewsupdateForm">
                                  <div class="modal-content-wrapper">
                                      ...
                                      <div class="form-section">
                                          ...
                                          <div class="form-group">
                                              <label for="newsupdateTitle" class="form-label">Title</label>
                                              <input type="text" id="newsupdateTitle" name="newsupdateTitle" class="form-control" placeholder="Input news/update title here..." required>
                                          </div>
                                          <div class="form-group">
                                              <label for="newsupdateDescription" class="form-label">Description</label>
                                              <textarea id="newsupdateDescription" name="newsupdateDescription" class="form-control textarea" rows="8" placeholder="Enter news/update description..." required></textarea>
                                          </div>
                                      </div>
                          ...
                          <div class="modal-footer">
                              <button type="submit" id="saveAddBtn" class="btn btn-primary" form="addNewsupdateForm">Save News/Update</button>
...
customElements.define("add-newsupdate-modal", AddNewsupdateModal);
