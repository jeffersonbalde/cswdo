class AddEmergencyOfficialsModal extends HTMLElement {
  constructor() {
    super();
  ...
  initializeElements() {
    this.modal = this.shadowRoot.getElementById("addEmergencyOfficialsModal");
    this.form = this.shadowRoot.getElementById("addEmergencyOfficialsForm");
    ...
    this.errorDiv = this.shadowRoot.getElementById("addEmergencyOfficialsError");
  }
  ...
  hasUserInput() {
    ...
    // Department is auto-filled to "em"; ignore it for user input detection
  ...
  async fetchNewEmployeeId() {
    try {
      ...
      formData.append('dept', 'em');
      const response = await fetch('./php_folder/manageEmergencyOfficials.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: formData
      });
      const text = await response.text();
      console.log("🪵 Raw response from PHP (EM):", text);
      let json;
      try { json = JSON.parse(text); } catch (err) { this.showError("Server returned invalid response. Check backend."); return; }
      if (json.success) { this.employeeId.value = json.employeeId; this.employeeId.readOnly = true; }
      else { this.employeeId.value = 1; this.employeeId.readOnly = true; this.showError("Failed to load Employee ID. Using default ID."); }
    } catch (err) {
      console.error("Error fetching ID:", err);
      this.employeeId.value = 1; this.employeeId.readOnly = true; this.showError("Could not fetch Employee ID. Using default ID.");
    }
  }

  async handleSubmit() {
    const formData = new FormData(this.form);
    if (this.selectedFile) formData.append("officialImage", this.selectedFile);
    const data = Object.fromEntries(formData.entries());
    if (!data.employeeId || !data.employeeName || !data.position || !data.department) { this.showError("Please fill in all required fields."); return; }
    if (!this.selectedFile) { this.showError("Please select an official image."); return; }
    ...
    confirmModal.addEventListener("confirm-save", async (e) => {
      if (e.detail.confirmed) {
        ...
        try {
          ...
          console.log("🔄 Sending EM form data:");
          ...
          console.log("📥 Raw response from server (EM):", responseText);
          let result;
          try {
  ...
  render() {
    this.shadowRoot.innerHTML = `
              ...
              <div id="addEmergencyOfficialsModal" class="modal">
                  <div class="modal-dialog">
                      <div class="modal-content">
                          <div class="modal-bg-logo"></div>
                          <div class="modal-header">  
                              <h5 id="addEmergencyOfficialsTitle" class="modal-title">Add Emergency Welfare Official</h5>
                              <button type="button" id="closeAddModal" class="btn-close">&times;</button>
                          </div>
                          <div class="modal-body">
                              <div id="addEmergencyOfficialsError" class="alert alert-danger"></div>
                              <form id="addEmergencyOfficialsForm">
                                      ...
                                      <div class="form-section">
                                          <div class="form-row">
                                              ...
                                              <div class="form-group">
                                                  ...
                                                  <input type="text" id="department" name="department" class="form-control" value="em" required readonly>
                                              </div>
                                          </div>
                                          <div class="form-row">
                                              <div class="form-group">
                          ...
                          <div class="modal-footer">
                              <button type="submit" id="saveAddBtn" class="btn btn-primary" form="addEmergencyOfficialsForm">Save Official</button>
...
customElements.define("add-emergency-officials-modal", AddEmergencyOfficialsModal);
