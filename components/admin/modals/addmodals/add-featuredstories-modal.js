class AddFeaturedstoriesModal extends HTMLElement {
    constructor() {
      super();
    ...
    async connectedCallback() {
      ...
      await this.fetchNewFeaturedstoriesId(); // 🆕 fetch from backend
    ...
    disconnectedCallback() {
      ...
      this.modal = this.shadowRoot.getElementById("addFeaturedstoriesModal");
      this.form = this.shadowRoot.getElementById("addFeaturedstoriesForm");
      this.cancelBtn = this.shadowRoot.getElementById("cancelAddBtn");
      this.closeBtn = this.shadowRoot.getElementById("closeAddModal");
      //Inputs
      this.featuredstoriesId = this.shadowRoot.getElementById("featuredstoriesId"); // Added for Featured Stories ID
      ...
      this.errorDiv = this.shadowRoot.getElementById("addFeaturedstoriesError");
    }
    ...
    hasUserInput() {
      // Featured Stories ID and upload date are auto-filled; ignore them for user input detection
      const titleHas = (this.featuredstoriesTitle?.value || "").trim().length > 0;
      const descHas = (this.featuredstoriesDesc?.value || "").trim().length > 0;
      const imageHas = !!this.selectedFile;
      return titleHas || descHas || imageHas;
    }
    ...
    setCurrentDateTime() {
      ...
      // Create display format: "September 3, 2024"
      const displayDate = `${month} ${day}, ${year}`;
      
      // Save full datetime to database but display formatted date to user
      const dateTimeString = now.toISOString();
      this.uploadDate.value = dateTimeString;
      this.uploadDate.readOnly = true;
      
      // Create a display-only element for the date
      const dateDisplay = this.shadowRoot.getElementById("dateDisplay");
      if (dateDisplay) {
        dateDisplay.textContent = displayDate;
      }
    ...
    //getnewID
    async fetchNewFeaturedstoriesId() {
      try {
        ...
        if (json.success) {
          this.featuredstoriesId.value = json.featuredstoriesId;
          this.featuredstoriesId.readOnly = true;
        } else {
          this.showError("Failed to load Featured Stories ID.");
        }
      } catch (err) {
        console.error("Error fetching ID:", err);
        this.showError("Could not get Featured Stories ID.");
      }
    }
    ...
    async handleSubmit() {
      ...
      if (this.selectedFile) {
        formData.append("featuredstoriesImage", this.selectedFile);
      }
      ...
      if (
        !data.featuredstoriesId ||
        !data.featuredstoriesTitle ||
        !data.uploadDate ||
        !data.featuredstoriesDescription
      ) {
      ...
      if (!this.selectedFile) {
        this.showError("Please select a featured story image.");
        return;
      }
        ...
        if (e.detail.confirmed) {
          ...
          try {
            ...
            if (result.success) {
              ...
              this.dispatchEvent(
                new CustomEvent("featuredstories-saved", {
                  detail: { featuredstories: data },
                  bubbles: true,
                })
              );
    ...
    render() {
      this.shadowRoot.innerHTML = `
                ...
                <div id="addFeaturedstoriesModal" class="modal">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-bg-logo"></div>
                            <div class="modal-header">  
                                <h5 id="addFeaturedstoriesTitle" class="modal-title">Add Featured Stories</h5>
                                <button type="button" id="closeAddModal" class="btn-close">&times;</button>
                            </div>
                            <div class="modal-body">
                                <div id="addFeaturedstoriesError" class="alert alert-danger"></div>
                                <form id="addFeaturedstoriesForm">
                                    <div class="modal-content-wrapper">
                                        ...
                                        <div class="form-section">
                                            ...
                                            <div class="form-group">
                                                <label for="featuredstoriesTitle" class="form-label">Title</label>
                                                <input type="text" id="featuredstoriesTitle" name="featuredstoriesTitle" class="form-control" placeholder="Input featured story title here..." required>
                                            </div>
                                            <div class="form-group">
                                                <label for="featuredstoriesDescription" class="form-label">Description</label>
                                                <textarea id="featuredstoriesDescription" name="featuredstoriesDescription" class="form-control textarea" rows="8" placeholder="Enter featured story description..." required></textarea>
                                            </div>
                                        </div>
                            ...
                            <div class="modal-footer">
                                <button type="submit" id="saveAddBtn" class="btn btn-primary" form="addFeaturedstoriesForm">Save Featured Story</button>
  ...
  customElements.define("add-featuredstories-modal", AddFeaturedstoriesModal);
