class ViewFCOfficialsModal extends HTMLElement {
  constructor() {
    super();
  ...
  initializeElements() {
    this.modal = this.shadowRoot.getElementById("viewFCOfficialModal");
    this.form = this.shadowRoot.getElementById("viewFCOfficialForm");
    ...
    this.errorDiv = this.shadowRoot.getElementById("viewFCOfficialError");
  }

  initEventListeners() {
    // Close modal events - use handleCloseAttempt to check for unsaved changes
    this.cancelBtn.addEventListener("click", () => { this.handleCloseAttempt(); });
    this.closeBtn.addEventListener("click", () => { this.handleCloseAttempt(); });

    this.modal.addEventListener("click", (e) => { if (e.target === this.modal) this.handleCloseAttempt(); });

    document.addEventListener("keydown", (e) => { if (e.key === "Escape" && this.modal.classList.contains("show")) this.handleCloseAttempt(); });

    this.form.addEventListener("submit", (e) => { e.preventDefault(); this.handleSubmit(); });

    ["employeeName", "position"].forEach((key) => { const input = this[key]; if (input) input.addEventListener("input", () => this.checkFormChanges()); });

    if (this.imageInput) this.imageInput.addEventListener("change", (e) => this.handleImageSelect(e));
    if (this.attachBtn) this.attachBtn.addEventListener("click", () => this.imageInput && this.imageInput.click());
  }
  ...
  handleImageSelect(event) {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) { this.showError("Please select a valid image file (JPG, PNG, or GIF)."); this.imageInput.value = ""; this.updateFilePath(""); return; }
      if (file.size > 5 * 1024 * 1024) { this.showError("File size too large. Maximum size is 5MB."); this.imageInput.value = ""; this.updateFilePath(""); return; }
      this.selectedFile = file; this.updateFilePath(file.name);
      const reader = new FileReader();
      reader.onload = (e) => { if (this.imagePreview) { this.imagePreview.src = e.target.result; this.imagePreview.style.display = "block"; } if (this.currentImage) this.currentImage.style.display = "none"; };
      reader.readAsDataURL(file);
      this.checkFormChanges();
    }
  }

  updateFilePath(fileName) { const filePathInput = this.shadowRoot.getElementById("filePathInput"); if (filePathInput) filePathInput.value = fileName || ""; }

  extractFileName(filePath) { if (!filePath) return ""; const parts = filePath.split(/[\/\\]/); return parts[parts.length - 1] || filePath; }
  ...
  async fetchOfficialDetails(officialId) {
    try {
      ...
      const res = await fetch("./php_folder/manageFamComOfficials.php", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: form });
      const text = await res.text(); let json; try { json = JSON.parse(text); } catch (_) { this.showError("Invalid server response."); return; }

      if (json.success && json.data) {
        const data = json.data;
        this.employeeId.value = data.employeeId ?? "";
        this.department.value = data.dept ?? "Family and Community";
        this.employeeName.value = data.name ?? "";
        this.position.value = data.position ?? "";
        if (data.picture) { this.currentImage.src = data.picture; this.currentImage.style.display = "block"; } else { this.currentImage.style.display = "none"; }
        this.updateFilePath(this.extractFileName(data.picture));
        this.originalData = { ...data };
        if (this.saveBtn) this.saveBtn.disabled = true;
      } else { this.showError(json.message || "Official not found."); }
    } catch (err) { this.showError("Could not fetch official details."); }
  }
  ...
  checkFormChanges() {
    const currentData = { employeeId: (this.employeeId.value || "").trim(), name: (this.employeeName.value || "").trim(), position: (this.position.value || "").trim(), dept: (this.department.value || "").trim() };
    if (!this.saveBtn) return; this.saveBtn.disabled = true;
    for (const key of ["name", "position"]) { const orig = this.originalData[key] != null ? String(this.originalData[key]).trim() : ""; const curr = currentData[key]; if (curr !== orig) { this.saveBtn.disabled = false; return; } }
    if (this.selectedFile) { this.saveBtn.disabled = false; return; }
  }
  ...
  async handleSubmit() {
    ...
    const dept = (this.department.value || this.originalData.dept || "fc").trim();
    ...
    confirmModal.addEventListener("confirm-save", async (e) => {
      ...
      this.savedFormData = {
        ...
        name,
        position,
        dept: dept || this.originalData.dept || 'fc',
      ...
      try {
        ...
        submitFormData.append("dept", dept || this.originalData.dept || 'fc');
        ...
        if (result.success) {
          ...
          successModal.setAttribute("message", "✅ Official Updated Successfully!");
  ...
  render() {
    this.shadowRoot.innerHTML = `
      ...
      <style>
        .modal { display: none; position: fixed; z-index: 1055; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; }
      ...
      <div id="viewFCOfficialModal" class="modal">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-bg-logo"></div>
            <div class="modal-header">
              <h5 id="viewFCOfficialTitle" class="modal-title">View FC Official</h5>
              <button type="button" id="closeViewModal" class="btn-close">&times;</button>
            </div>
            <div class="modal-body">
              <div id="viewFCOfficialError" class="alert alert-danger"></div>
              <form id="viewFCOfficialForm">
                  ...
                  <div class="form-section">
                    <div class="form-row">
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" id="department" name="department" class="form-control" value="Family and Community" readonly>
                      </div>
                    </div>
                    <div class="form-row">
                      <div class="form-group">
            ...
            <div class="modal-footer">
              <button type="submit" id="saveChangeBtn" class="btn btn-primary" form="viewFCOfficialForm" disabled>Save changes</button>
...
customElements.define("view-famcom-officials-modal", ViewFCOfficialsModal);
