class ViewCWOfficialsModal extends HTMLElement {
  constructor() {
    super();
  ...
  initializeElements() {
    this.modal = this.shadowRoot.getElementById("viewCWOfficialModal");
    this.form = this.shadowRoot.getElementById("viewCWOfficialForm");
    ...
    this.errorDiv = this.shadowRoot.getElementById("viewCWOfficialError");
  }
  ...
  initEventListeners() {
    ...
    ["employeeName", "position"].forEach((key) => {
      ...
      if (input) input.addEventListener("input", () => this.checkFormChanges());
    });
  ...
  async fetchOfficialDetails(officialId) {
    try {
      ...
      const res = await fetch("./php_folder/manageCWOfficials.php", {
        method: "POST",
      ...
      let json;
      try { json = JSON.parse(text); } catch (_) { this.showError("Invalid server response."); return; }

      if (json.success && json.data) {
        const data = json.data;
        this.employeeId.value = data.employeeId ?? "";
        this.department.value = data.dept ?? "Child and Youth Welfare";
  ...
  checkFormChanges() {
    ...
    for (const key of ["name", "position"]) {
      const orig = this.originalData[key] != null ? String(this.originalData[key]).trim() : "";
      const curr = currentData[key];
      if (curr !== orig) { this.saveBtn.disabled = false; return; }
    }
  ...
  async handleSubmit() {
    ...
    const dept = (this.department.value || this.originalData.dept || "cw").trim();
    ...
    confirmModal.addEventListener("confirm-save", async (e) => {
      ...
      this.savedFormData = {
        ...
        name,
        position,
        dept: dept || this.originalData.dept || 'cw',
      ...
      try {
        ...
        submitFormData.append("dept", dept || this.originalData.dept || 'cw');
        // preserve existing picture if not changed
        submitFormData.append("picture", this.originalData.picture || "");
        if (this.selectedFile) {
          submitFormData.append("officialImage", this.selectedFile);
        }

        const response = await fetch("./php_folder/manageCWOfficials.php", {
          method: "POST",
          body: submitFormData
        });
  ...
  render() {
    this.shadowRoot.innerHTML = `
      ...
      <div id="viewCWOfficialModal" class="modal">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-bg-logo"></div>
            <div class="modal-header">
              <h5 id="viewCWOfficialTitle" class="modal-title">View CW Official</h5>
              <button type="button" id="closeViewModal" class="btn-close">&times;</button>
            </div>
            <div class="modal-body">
              <div id="viewCWOfficialError" class="alert alert-danger"></div>
              <form id="viewCWOfficialForm">
                  ...
                  <div class="form-section">
                    <div class="form-row">
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" id="department" name="department" class="form-control" value="Child and Youth Welfare" readonly>
                      </div>
                    </div>
                    <div class="form-row">
                      <div class="form-group">
            ...
            <div class="modal-footer">
              <button type="submit" id="saveChangeBtn" class="btn btn-primary" form="viewCWOfficialForm" disabled>Save changes</button>
...
customElements.define("view-cw-officials-modal", ViewCWOfficialsModal);
