class AboutUsEmergencyOfficials extends HTMLElement {
  constructor() {
    super();
  ...
  async fetchAndRenderOfficials() {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log("Loaded EM officials:", this.officials.length);
      } else {
        console.error("Failed to fetch EM officials:", result.message);
        this.officials = [];
  ...
  async showAddOfficialModal() {
    const modal = document.createElement("add-emergency-officials-modal");
    document.body.appendChild(modal);
    modal.addEventListener("official-saved", (e) => { console.log("EM Official saved:", e.detail); });
    modal.addEventListener("refresh-officials-table", () => { this.fetchAndRenderOfficials(); });
  }

  async viewOfficial(officialId) {
    try {
      const existing = document.querySelector('view-emergency-officials-modal');
      if (existing) existing.remove();
      const modal = document.createElement('view-emergency-officials-modal');
      document.body.appendChild(modal);
      if (customElements && customElements.whenDefined) {
        customElements.whenDefined('view-emergency-officials-modal').then(() => {
      ...
      console.error('Failed to open EM view modal:', e);
  ...
  async editOfficial(officialId) {
    try {
      ...
      const result = await response.json();
      if (result.success) {
        console.log("EM Official data for editing:", result.data);
      ...
      console.error("Error editing EM official:", err);
  ...
  async deleteOfficial(officialId) {
    ...
    try {
      ...
      console.error("Error deleting EM official:", err);
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="about-container">
        ...
        <div class="about-content">
          <!-- Section Label -->
          <h2 class="section-label">Emergency Welfare Officials</h2>
...
customElements.define("about-us-em-officials", AboutUsEmergencyOfficials)
