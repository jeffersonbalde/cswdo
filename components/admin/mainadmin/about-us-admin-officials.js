class AboutUsAdminOfficials extends HTMLElement {
  constructor() {
    super();
  ...
  async fetchAndRenderOfficials() {
    ...
    try {
      ...
      formData.append('dept', 'admin');
      
      const response = await fetch('./php_folder/manageAdminOfficials.php', {
        method: 'POST',
      ...
      if (result.success) {
        ...
        console.log("Loaded officials:", this.officials.length);
      } else {
        console.error("Failed to fetch officials:", result.message);
        this.officials = [];
  ...
  async addNewOfficial(official) {
    try {
      const formData = new URLSearchParams();
      formData.append('action', 'save');
      formData.append('employeeId', official.employeeId || '');
      formData.append('name', official.name || '');
      formData.append('position', official.position || '');
      formData.append('picture', official.picture || '');
      formData.append('dept', 'admin');
      ...
      if (result.success) {
        ...
        await this.fetchAndRenderOfficials();
        this.showNotification(result.message, "success");
      } else {
        this.showNotification(result.message || "Failed to add official.", "error");
      }
    } catch (err) {
      console.error("Error adding official:", err);
      this.showNotification("Failed to add official.", "error");
    }
  }

  async updateOfficial(updatedOfficial) {
    try {
      const formData = new URLSearchParams();
      formData.append('action', 'update');
      formData.append('official_id', updatedOfficial.id);
      formData.append('employeeId', updatedOfficial.employeeId || '');
      formData.append('name', updatedOfficial.name || '');
      formData.append('position', updatedOfficial.position || '');
      formData.append('picture', updatedOfficial.picture || '');
      formData.append('dept', 'admin');
      ...
      if (result.success) {
        ...
        this.showNotification(result.message || "Failed to update official.", "error");
      }
    } catch (err) {
      console.error("Error updating official:", err);
      this.showNotification("Failed to update official.", "error");
    }
  }


  async showAddOfficialModal() {
    // Create and open the add official modal
    const modal = document.createElement("add-admin-officials-modal");
    document.body.appendChild(modal);

    // Listen for successful save
    modal.addEventListener("official-saved", (e) => {
      console.log("Official saved:", e.detail);
    });

    // Listen for table refresh
    modal.addEventListener("refresh-officials-table", () => {
      this.fetchAndRenderOfficials();
    });
  }
  ...
  async viewOfficial(officialId) {
    try {
      const existing = document.querySelector('view-admin-officials-modal');
      if (existing) existing.remove();
      const modal = document.createElement('view-admin-officials-modal');
      document.body.appendChild(modal);
      if (customElements && customElements.whenDefined) {
        customElements.whenDefined('view-admin-officials-modal').then(() => {
      ...
      console.error('Failed to open Admin view modal:', e);
  ...
  async editOfficial(officialId) {
    try {
      ...
      if (result.success) {
        // TODO: Open edit modal with the official data
        console.log("Official data for editing:", result.data);
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="about-container">
        ...
        <div class="about-content">
          <!-- Section Label -->
          <h2 class="section-label">Administrative Services Officials</h2>
...
customElements.define("about-us-admin-officials", AboutUsAdminOfficials)
