class AboutUsFCOfficials extends HTMLElement {
  constructor() {
    super();
  ...
  async fetchAndRenderOfficials() {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log("Loaded FC officials:", this.officials.length);
      } else {
        console.error("Failed to fetch FC officials:", result.message);
        this.officials = [];
  ...
  async showAddOfficialModal() {
    const modal = document.createElement("add-famcom-officials-modal");
    document.body.appendChild(modal);

    modal.addEventListener("official-saved", (e) => {
      console.log("FC Official saved:", e.detail);
    });
  ...
  viewOfficial(officialId) {
    try {
      const existing = document.querySelector('view-famcom-officials-modal');
      if (existing) existing.remove();
      const modal = document.createElement('view-famcom-officials-modal');
      document.body.appendChild(modal);
      if (customElements && customElements.whenDefined) {
        customElements.whenDefined('view-famcom-officials-modal').then(() => {
      ...
      console.error('Failed to open FC view modal:', e);
  ...
  async editOfficial(officialId) {
    try {
      ...
      if (result.success) {
        console.log("FC Official data for editing:", result.data);
      ...
      console.error("Error editing FC official:", err);
  ...
  async deleteOfficial(officialId) {
    ...
    try {
      ...
      console.error("Error deleting FC official:", err);
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="about-container">
        ...
        <div class="about-content">
          <!-- Section Label -->
          <h2 class="section-label">Family and Community Officials</h2>
...
customElements.define("about-us-fc-officials", AboutUsFCOfficials)
