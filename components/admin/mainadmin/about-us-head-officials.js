class AboutUsHeadOfficials extends HTMLElement {
  constructor() {
    super();
  ...
  setupEventListeners() {
    ...
    // Note: consolidated click handling exists below (prevents duplicate triggers)
    ...
    // Add Official button -> open add modal
    const addOfficialBtn = this.shadowRoot.querySelector("#addOfficialBtn");
    if (addOfficialBtn) {
  ...
  async fetchAndRenderOfficials() {
    ...
    try {
      ...
      body.append('action', 'fetchdata');
      body.append('dept', 'Head Official');
      ...
      try {
        ...
        console.error('Invalid JSON from manageHeadOfficials.php:', text);
        throw new Error('Invalid server response');
      }

      if (result && result.success) {
        this.officials = Array.isArray(result.data) ? result.data : [];
      } else {
        this.officials = [];
  ...
  async showAddOfficialModal() {
    const modal = document.createElement('add-head-officials-modal');
    // When the modal signals a save or refresh, update the table
    modal.addEventListener('official-saved', () => {
      this.refreshTable();
    });
  ...
  viewOfficial(officialId) {
    try {
      const existing = document.querySelector('view-head-officials-modal');
      if (existing) existing.remove();
      const modal = document.createElement('view-head-officials-modal');
      document.body.appendChild(modal);
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="about-container">
        ...
        <div class="about-content">
          <!-- Section Label -->
          <h2 class="section-label">Head Officials</h2>
...
customElements.define("about-us-head-officials", AboutUsHeadOfficials)
