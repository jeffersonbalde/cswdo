class AboutUsCWOfficials extends HTMLElement {
  constructor() {
    super();
    ...
    this.componentMap = {
      ...
      "about-us-emergency-officials": {
        ...
        component: "about-us-emergency-officials",
      },
  ...
  async fetchAndRenderOfficials() {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log("Loaded CW officials:", this.officials.length);
      } else {
        console.error("Failed to fetch CW officials:", result.message);
        this.officials = [];
  ...
  async showAddOfficialModal() {
    const modal = document.createElement("add-cw-officials-modal");
    document.body.appendChild(modal);

    modal.addEventListener("official-saved", (e) => {
      console.log("CW Official saved:", e.detail);
    });
  ...
  viewOfficial(officialId) {
    try {
      const existing = document.querySelector('view-cw-officials-modal');
      if (existing) existing.remove();
      const modal = document.createElement('view-cw-officials-modal');
      document.body.appendChild(modal);
      // Ensure the custom element definition is loaded before calling methods
      if (customElements && customElements.whenDefined) {
        customElements.whenDefined('view-cw-officials-modal').then(() => {
          if (typeof modal.setOfficialId === 'function') {
            modal.setOfficialId(officialId);
          } else {
            console.error('view-cw-officials-modal not upgraded, retrying...');
      ...
      console.error('Failed to open CW view modal:', e);
  ...
  async editOfficial(officialId) {
    try {
      ...
      if (result.success) {
        console.log("CW Official data for editing:", result.data);
      ...
      console.error("Error editing CW official:", err);
  ...
  async deleteOfficial(officialId) {
    ...
    try {
      ...
      console.error("Error deleting CW official:", err);
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="about-container">
        ...
        <div class="about-content">
          <!-- Section Label -->
          <h2 class="section-label">Child and Youth Welfare Officials</h2>
...
customElements.define("about-us-cw-officials", AboutUsCWOfficials)
