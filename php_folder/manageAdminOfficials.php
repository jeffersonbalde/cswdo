...
if (!$conn) {
    error_log("Database connection failed in manageAdminOfficials.php");
    echo json_encode([
        'success' => false,
...
// Check if the manageadminofficials table exists
$tableCheck = $conn->query("SHOW TABLES LIKE 'manageadminofficials'");
if ($tableCheck->num_rows === 0) {
    error_log("Table 'manageadminofficials' does not exist in database");
    echo json_encode([
        'success' => false,
        'message' => 'Database table not found. Please contact administrator.',
        'error_details' => 'Table manageadminofficials does not exist'
    ]);
    ob_end_flush();
    exit;
}
...
// Debug logging
error_log("manageAdminOfficials.php called with POST data: " . json_encode($_POST));
error_log("manageAdminOfficials.php called with FILES data: " . json_encode($_FILES));
...
if ($action === "getNextEmployeeId") {
    try {
        $result = $conn->query("SELECT MAX(official_employee_id) as max_id FROM manageadminofficials");
...
elseif ($action === "fetchdata") {
    try {
        ...
        if ($dept) {
            $sql = "SELECT * FROM manageadminofficials WHERE official_dept = '$dept' ORDER BY official_employee_id DESC";
        } else {
            $sql = "SELECT * FROM manageadminofficials ORDER BY official_employee_id DESC";
        }
...
elseif ($action === "getOfficialById") {
    ...
    try {
        $sql = "SELECT * FROM manageadminofficials WHERE official_employee_id = '$id'";
        $result = $conn->query($sql);
...
elseif ($action === "delete") {
    ...
    try {
        $sql = "DELETE FROM manageadminofficials WHERE official_employee_id = '$id'";
...
elseif ($action === "save") {
    ...
    if (isset($_FILES['officialImage']) && $_FILES['officialImage']['error'] === UPLOAD_ERR_OK) {
        ...
        $uploadDir = '../uploads/AdminOfficialsPictures/';  // Using servicephotos folder
        if (!is_dir($uploadDir)) {
        ...
        if (move_uploaded_file($file['tmp_name'], $targetPath)) {
            $picturePath = '/uploads/AdminOfficialsPictures/' . $fileName;
        } else {
            echo json_encode([
                'success' => false,
    ...
    $dept          = $conn->real_escape_string($_POST["dept"] ?? 'admin');
    ...
    try {
        ...
        $sql = "INSERT INTO manageadminofficials (
...
elseif ($action === "update") {
    ...
    try {
        ...
        if (isset($_FILES['officialImage']) && $_FILES['officialImage']['error'] === UPLOAD_ERR_OK) {
            ...
            $uploadDir = '../uploads/AdminOfficialsPictures/';
            if (!is_dir($uploadDir)) {
                mkdir($uploadDir, 0755, true);
            }
        ...
        $sql = "UPDATE manageadminofficials SET 
                    official_name = '$name',
