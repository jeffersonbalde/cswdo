...
if (!$conn) {
    error_log("Database connection failed in manageEmergencyOfficials.php");
...
// Ensure table exists
$tableCheck = $conn->query("SHOW TABLES LIKE 'manageemergencyofficials'");
if ($tableCheck->num_rows === 0) {
    error_log("Table 'manageemergencyofficials' does not exist in database");
    echo json_encode([
        'success' => false,
        'message' => 'Database table not found. Please contact administrator.',
        'error_details' => 'Table manageemergencyofficials does not exist'
    ]);
    ob_end_flush();
    exit;
}
...
error_log("manageEmergencyOfficials.php called with POST data: " . json_encode($_POST));
error_log("manageEmergencyOfficials.php called with FILES data: " . json_encode($_FILES));
...
if ($action === "getNextEmployeeId") {
    try {
        $result = $conn->query("SELECT MAX(official_employee_id) as max_id FROM manageemergencyofficials");
        if ($result) {
        ...
        error_log("Error getting next employee ID (EM): " . $e->getMessage());
        echo json_encode(['success' => true, 'employeeId' => 1]);
    }
    ob_end_flush();
    exit;
...
elseif ($action === "fetchdata") {
    try {
        ...
        $sql = $dept
            ? "SELECT * FROM manageemergencyofficials WHERE official_dept = '$dept' ORDER BY official_employee_id DESC"
            : "SELECT * FROM manageemergencyofficials ORDER BY official_employee_id DESC";
        ...
        error_log("Error fetching EM officials: " . $e->getMessage());
...
elseif ($action === "getOfficialById") {
    ...
    try {
        $sql = "SELECT * FROM manageemergencyofficials WHERE official_employee_id = '$id'";
        $result = $conn->query($sql);
        ...
        error_log("Error getting EM official by ID: " . $e->getMessage());
...
elseif ($action === "delete") {
    ...
    try {
        $sql = "DELETE FROM manageemergencyofficials WHERE official_employee_id = '$id'";
        if ($conn->query($sql)) {
        ...
        error_log("Error deleting EM official: " . $e->getMessage());
...
elseif ($action === "save") {
    ...
    if (isset($_FILES['officialImage']) && $_FILES['officialImage']['error'] === UPLOAD_ERR_OK) {
        ...
        $uploadDir = '../uploads/EmergencyOfficialsPictures/';
        if (!is_dir($uploadDir)) { mkdir($uploadDir, 0755, true); }
        $fileName = time() . '_' . basename($file['name']);
        $targetPath = $uploadDir . $fileName;
        if (move_uploaded_file($file['tmp_name'], $targetPath)) {
            $picturePath = '/uploads/EmergencyOfficialsPictures/' . $fileName;
        } else {
    ...
    $dept = $conn->real_escape_string($_POST["dept"] ?? 'em');
    ...
    try {
        ...
        $sql = "INSERT INTO manageemergencyofficials (
        ...
        error_log("Error saving EM official: " . $e->getMessage());
...
elseif ($action === "update") {
    ...
    try {
        ...
        $sql = "UPDATE manageemergencyofficials SET 
                    official_name = '$name',
        ...
        error_log("Error updating EM official: " . $e->getMessage());
