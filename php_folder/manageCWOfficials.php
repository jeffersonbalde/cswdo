...
if (!$conn) {
    error_log("Database connection failed in manageCWOfficials.php");
    echo json_encode([
        'success' => false,
...
// Ensure table exists
$tableCheck = $conn->query("SHOW TABLES LIKE 'managecwofficials'");
if ($tableCheck->num_rows === 0) {
    error_log("Table 'managecwofficials' does not exist in database");
    echo json_encode([
        'success' => false,
        'message' => 'Database table not found. Please contact administrator.',
        'error_details' => 'Table managecwofficials does not exist'
    ]);
    ob_end_flush();
    exit;
}
...
error_log("manageCWOfficials.php called with POST data: " . json_encode($_POST));
error_log("manageCWOfficials.php called with FILES data: " . json_encode($_FILES));
...
if ($action === "getNextEmployeeId") {
    try {
        $result = $conn->query("SELECT MAX(official_employee_id) as max_id FROM managecwofficials");
        if ($result) {
        ...
        error_log("Error getting next employee ID (CW): " . $e->getMessage());
        echo json_encode([
            'success' => true,
            'employeeId' => 1
        ]);
...
elseif ($action === "fetchdata") {
    try {
        ...
        if ($dept) {
            $sql = "SELECT * FROM managecwofficials WHERE official_dept = '$dept' ORDER BY official_employee_id DESC";
        } else {
            $sql = "SELECT * FROM managecwofficials ORDER BY official_employee_id DESC";
        }
        ...
        error_log("Error fetching CW officials: " . $e->getMessage());
        echo json_encode([
            'success' => false,
...
elseif ($action === "getOfficialById") {
    ...
    try {
        $sql = "SELECT * FROM managecwofficials WHERE official_employee_id = '$id'";
        $result = $conn->query($sql);
        ...
        error_log("Error getting CW official by ID: " . $e->getMessage());
        echo json_encode([
            'success' => false,
...
elseif ($action === "delete") {
    ...
    try {
        $sql = "DELETE FROM managecwofficials WHERE official_employee_id = '$id'";
        if ($conn->query($sql)) {
            echo json_encode([
                'success' => true,
        ...
        error_log("Error deleting CW official: " . $e->getMessage());
        echo json_encode([
            'success' => false,
...
elseif ($action === "save") {
    ...
    if (isset($_FILES['officialImage']) && $_FILES['officialImage']['error'] === UPLOAD_ERR_OK) {
        ...
        $uploadDir = '../uploads/ChildYouthWelfareOfficialsPictures/';
        if (!is_dir($uploadDir)) {
        ...
        if (move_uploaded_file($file['tmp_name'], $targetPath)) {
            $picturePath = '/uploads/ChildYouthWelfareOfficialsPictures/' . $fileName;
        } else {
            echo json_encode([
                'success' => false,
    ...
    $dept = $conn->real_escape_string($_POST["dept"] ?? 'cw');
    ...
    try {
        ...
        $sql = "INSERT INTO managecwofficials (
        ...
        error_log("Error saving CW official: " . $e->getMessage());
        echo json_encode([
            'success' => false,
...
elseif ($action === "update") {
    ...
    try {
        // Default to existing path unless a new file is uploaded
        ...
        $sql = "UPDATE managecwofficials SET 
                    official_name = '$name',
        ...
        error_log("Error updating CW official: " . $e->getMessage());
        echo json_encode([
            'success' => false,
